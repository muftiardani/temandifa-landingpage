...
test.describe('Homepage', () => {
  ...
  test('should navigate to about page', async ({ page, isMobile }) => {
    if (isMobile) {
      ...
      const menuButton = page.getByTestId('mobile-menu-button').or(page.locator('button[aria-label*="menu"]')).first();
      await menuButton.waitFor({ state: 'visible', timeout: 10000 });
      await menuButton.click();
      await page.waitForTimeout(800);
      
      // Click mobile nav link
      const aboutLink = page.getByTestId('mobile-nav-about').or(page.locator('a[href*="tentang"]')).first();
      await aboutLink.waitFor({ state: 'visible', timeout: 10000 });
      await aboutLink.click();
    } else {
      // Desktop: Click nav link directly
      const aboutLink = page.getByTestId('nav-about').or(page.locator('a[href*="tentang"]')).first();
      await aboutLink.waitFor({ state: 'visible', timeout: 10000 });
      await aboutLink.click();
    }
  ...
  test('should navigate to product page', async ({ page, isMobile }) => {
    if (isMobile) {
      ...
      // Click mobile nav link
      const productLink = page.getByTestId('mobile-nav-features').or(page.locator('a[href*="produk"]')).first();
      await productLink.waitFor({ state: 'visible', timeout: 10000 });
      await productLink.click();
    } else {
      // Desktop: Click nav link directly
      const productLink = page.getByTestId('nav-features').or(page.locator('a[href*="produk"]')).first();
      await productLink.waitFor({ state: 'visible', timeout: 10000 });
      await productLink.click();
    }
  ...
  test('should navigate to contact page', async ({ page, isMobile }) => {
    if (isMobile) {
      ...
      // Click mobile nav link
      const contactLink = page.getByTestId('mobile-nav-contact').or(page.locator('a[href*="kontak"]')).first();
      await contactLink.waitFor({ state: 'visible', timeout: 10000 });
      await contactLink.click();
    } else {
      // Desktop: Click nav link directly
      const contactLink = page.getByTestId('nav-contact').or(page.locator('a[href*="kontak"]')).first();
      await contactLink.waitFor({ state: 'visible', timeout: 10000 });
      await contactLink.click();
    }
